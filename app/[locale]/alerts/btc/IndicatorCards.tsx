...
type IndicatorCardState = 'ON' | 'OFF' | 'PARTIAL'

function getIndicatorCardState(
  value: {
    '15m': boolean
    '1h': boolean
  },
...
export default function IndicatorCards() {
  ...
  const toggle = async (key: IndicatorKey) => {
    if (!notificationSettings || savingKey) {
      return
    }

    const currentState =
      getIndicatorCardState(
        notificationSettings.indicatorEnabled[
          key
        ],
      )
    ...
    try {
      await saveNotificationSettings({
        indicatorEnabled: indicatorPatch,
      })
