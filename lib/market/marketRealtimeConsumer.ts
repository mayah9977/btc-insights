...
const DERIVED_PUBLIC =
  'realtime:derived:public'

const DERIVED_VIP =
  'realtime:derived:vip'

const DEFAULT_SYMBOLS = [
  'BTCUSDT',
]

const PRICE_PUBLISH_INTERVAL_MS =
  100
...
if (!g.__MARKET_CONSUMER_STARTED__) {
  ...
  const lastWhalePublish =
    new Map<string, number>()

  const lastPricePublishAt =
    new Map<string, number>()

  const pendingPriceEvent =
    new Map<string, any>()
      ...
      try {
        ...
        if (
          ...
          if (
            ...
            lastPricePublishAt.set(
              symbol,
              now,
            )

            const latest =
              pendingPriceEvent.get(
                symbol,
              ) ?? event

            pendingPriceEvent.delete(
              symbol,
            )
          ...
          if (
            ...
            setLastBollingerSignal(
              symbol,
              {
                ...
                symbol,
                signalType:
                  result.confirmedSignal.signalType as BollingerSignalType,
                candle:
                  result.confirmedSignal
                    .candle,
                bands:
            ...
            if (
              ...
              setLastBollingerSignal(
                symbol,
                {
                  ...
                  symbol,
                  signalType:
                    result.liveCommentary.signalType as BollingerSignalType,
                  candle:
                    result.liveCommentary
                      .candle,
                  bands:
  ...
  setInterval(async () => {
    ...
    for (const symbol of activeSymbols) {
      ...
      try {
        ...
        const snapshot =
          await buildRiskInputFromRealtime(
            symbol,
          )
        ...
        const absorption =
          detectWhaleAbsorption({
            priceChange,
            whaleBuyVolume:
              snapshot.whaleBuyVolume ??
              0,
            whaleSellVolume:
              snapshot.whaleSellVolume ??
              0,
            whaleRatio:
              snapshot.whaleRatio ??
              0,
        ...
        setInstitutionalState(
          symbol,
          {
            detected:
              institutional.detected,
            direction:
        ...
        const nextActionGateInput:
          ActionGateInput = {
            bollingerSignal:
              snapshot.bollingerSignal,
            oiDelta:
              snapshot.oiDelta,
            oiDeltaRatio:
              snapshot.oiDeltaRatio,
