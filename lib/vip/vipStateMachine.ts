...
import { getUserVIPState, recoverVIP, forceExpireVIP } from './vipDB';
import { recordVIPChange, VIPAuditReason } from './vipAuditLog';
...
export async function transitionVIP(input: TransitionInput) {
  ...
  switch (input.toStatus) {
    case 'ACTIVE': {
      if (!input.level) {
        throw new Error('ACTIVE requires level');
      }
      await recoverVIP(
        input.userId,
        input.level,
    ...
    default:
      throw new Error(`Unsupported transition ${input.toStatus}`);
  }
}
